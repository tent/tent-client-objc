...
+ (instancetype)responseEnvelopeWithJSONDictionary:(NSDictionary *)responseJSON requestURL:(NSURL *)requestURL {
    return [[TCResponseEnvelope alloc] initWithResponseJSON:responseJSON requestURL:requestURL];
}

- (instancetype)initWithResponseJSON:(NSDictionary *)responseJSON requestURL:(NSURL *)requestURL {
    self = [super init];
    ...
    if ([self.responseJSON objectForKey:@"error"]) {
        self.error = [NSError errorWithDomain:TCResponseEnvelopeErrorDomain code:1 userInfo:self.responseJSON];
        return self;
    }
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@ responseJSON=%@>", self.class, [self.responseJSON description]];
}
...
- (NSURL *)firstPageURL {
    if (pages && [pages objectForKey:@"first"]) {
        return [NSURL URLWithString:[pages objectForKey:@"first"] relativeToURL:self.requestURL];
    } else {
...
- (NSURL *)previousPageURL {
    if (pages && [pages objectForKey:@"prev"]) {
        return [NSURL URLWithString:[pages objectForKey:@"prev"] relativeToURL:self.requestURL];
    } else {
...
- (NSURL *)nextPageURL {
    if (pages && [pages objectForKey:@"next"]) {
        return [NSURL URLWithString:[pages objectForKey:@"next"] relativeToURL:self.requestURL];
    } else {
...
- (NSURL *)lastPageURL {
    if (pages && [pages objectForKey:@"last"]) {
        return [NSURL URLWithString:[pages objectForKey:@"last"] relativeToURL:self.requestURL];
    } else {
...
- (NSArray *)posts {
    if (deserializedPosts) return deserializedPosts;
